...
public enum DialogueLayerMask
{
    ...
    // ---- Group masks (단순 그룹) ----
    AllBackgrounds  = Background0 | Background1,
    AllPortraits    = MainPortrait | SubLeftPortrait | SubRightPortrait,
    AllUI           = DialogueBox | ChoicePanel,

    // ---- VN Presets (실제 연출에서 자주 쓰는 상태들) ----
    // 1) 일반 스토리: 배경 1 + 대화창
    Story_Default   = Background0 | DialogueBox,

    // 2) 주로 나오는 패턴들
    Story_Solo      = Background0 | MainPortrait | DialogueBox,                           // 1인 화자
    Story_Duo       = Background0 | MainPortrait | SubLeftPortrait | DialogueBox,         // 2인 (메인 + 서브)
    Story_Trio      = Background0 | MainPortrait | SubLeftPortrait | SubRightPortrait | DialogueBox, // 3인

    // 3) 선택지: 보통 대화창 대신 선택지만 뜸
    Choice_Default  = Background0 | ChoicePanel,
